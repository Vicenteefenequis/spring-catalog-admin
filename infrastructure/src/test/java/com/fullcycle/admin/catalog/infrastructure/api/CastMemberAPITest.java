...
public class CastMemberAPITest {
    ...
    public void givenAValidCommand_whenCallsCreateCastMember_shouldReturnItsIdentifier() throws Exception {
        ...
        final var expectedId = CastMemberID.from("any_id");
        ...
        //when
        final var aRequest = post("/cast_members").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(aCommand));
        ...
        response.andExpect(status().isCreated()).andExpect(header().string("Location", "/cast_members/" + expectedId.getValue())).andExpect(header().string("Content-Type", MediaType.APPLICATION_JSON_VALUE)).andExpect(jsonPath("$.id", equalTo(expectedId.getValue())));
    ...
    public void givenAInvalidId_whenCallsGetByIdAndCastMemberDoesNotExists_shouldReturnNotFound() throws Exception {
        ...
        when(getCastMemberByIdUseCase.execute(any())).thenThrow(
    ...
    public void givenAValidCommand_whenCallsUpdateCastMember_shouldReturnItsIdentifier() throws Exception {
        ...
        //when
        final var aRequest = put("/cast_members/{id}", expectedId.getValue()).contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(aCommand));
    ...
    public void givenValidParams_whenCallListCastMembers_shouldReturnIt() throws Exception {
        ...
        when(listCastMembersUseCase.execute(any())).thenReturn(
                new Pagination<>(
                        expectedPage,
                        expectedPerPage,
                        expectedTotal,
        ...
        response.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.items[0].updated_at", equalTo(aMember.getUpdatedAt().toString())));
